...
multilib_src_compile()
{
	...
	if has_multilib_profile && [[ ${ABI} == "x86" ]] ; then
		escons --cache-disable DIST_TARGET="i686" LIBDIR="${EPREFIX}/usr/$(get_libdir)"
	else
...
{
	if has_multilib_profile && [[ ${ABI} == "x86" ]] ; then
		escons --cache-disable DIST_TARGET="i686" LIBDIR="${EPREFIX}/usr/$(get_libdir)" DESTDIR="${D}" WILL_DEAL_WITH_XDG_MYSELF="True" install
	else
